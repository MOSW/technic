...
function technic.register_battery_box(tier, max_charge, max_charge_rate, max_discharge_rate, charge_step, discharge_step)
...
for i = 0, 8 do
	...
	minetest.register_node("technic:"..ltier.."_battery_box"..i, {
		...
		tiles = {"technic_"..ltier.."_battery_box_top.png", "technic_"..ltier.."_battery_box_bottom.png",
		...
		on_construct = function(pos)
			...
			local tier = minetest.registered_nodes[node.name].battery_box.tier

			meta:set_string("infotext", tier.." Battery Box")
			meta:set_string("formspec", battery_box_formspec)
			meta:set_int(tier.."_EU_demand", 0) -- How much this node can charge
			meta:set_int(tier.."_EU_supply", 0) -- How much this node can discharge
			meta:set_int(tier.."_EU_input",  0) -- How much power this machine is getting.
...
minetest.register_abm({
	nodenames = {"technic:"..ltier.."_battery_box0", "technic:"..ltier.."_battery_box1", "technic:"..ltier.."_battery_box2",
	             "technic:"..ltier.."_battery_box3", "technic:"..ltier.."_battery_box4", "technic:"..ltier.."_battery_box5",
	             "technic:"..ltier.."_battery_box6", "technic:"..ltier.."_battery_box7", "technic:"..ltier.."_battery_box8"},
	interval = 1,
	chance   = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		local data           = minetest.registered_nodes[node.name].battery_box
		local meta           = minetest.get_meta(pos)
		...
		local max_charge_rate    = data.max_charge_rate
		local max_discharge_rate = data.max_discharge_rate
		...
		current_charge = technic.charge_tools(meta, current_charge, data.charge_step)
		current_charge = technic.discharge_tools(meta, current_charge, data.discharge_step, max_charge)

		-- Set a demand (we allow batteries to charge on less than the demand though)
		meta:set_int(data.tier.."_EU_demand", math.min(max_charge_rate, max_charge - current_charge))

		-- Set how much we can supply
		meta:set_int(data.tier.."_EU_supply", math.min(max_discharge_rate, current_charge))
		...
		if eu_input == 0 then
			meta:set_string("infotext", data.tier.." Battery box: "..current_charge.."/"..max_charge.." (idle)")
		else
			meta:set_string("infotext", data.tier.." Battery box: "..current_charge.."/"..max_charge)
		end
	end
})
